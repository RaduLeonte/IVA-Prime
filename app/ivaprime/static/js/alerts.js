const Alerts = new class {
    constructor () {
        // Alerts container that alerts are appended to
        this.alertsContainer;

        document.addEventListener("DOMContentLoaded", () => {
            // Store reference to alerts container
            Alerts.alertsContainer = document.getElementById("alerts-container");
    ...
    /**
     * Append alert as first child of the alerts container.
     * 
     * @param {HTMLElement} alert - Reference to alert to append
     */
    _appendAlert(alert) {
        ...
        this.alertsContainer.insertBefore(alert, this.alertsContainer.firstChild);
    };
    ...
    warning(title, body, lifetime=5, color="orange", ) {
        this.showAlert(title, body, lifetime, color);
    };
    ...
    showAlert(title, body, lifetime=5, color="orange", ) {
        ...
        alert.appendChild(alertIconContainer); // Append
        
        const alertIconSvgWrapper = document.createElement('div');
        alertIconSvgWrapper.classList.add("alert-icon-svg-wrapper");
        alertIconSvgWrapper.innerHTML = `
        ...
        closeButton.addEventListener('click', function (e) { Alerts._removeAlert(alert) });
        alert.appendChild(closeButton); // Append
        ...
        if (lifetime !== -1) {
            setTimeout(function (e) { Alerts._removeAlert(alert) }, lifetime*1000);
        };
    };
     ...
     * 
     * @param {HTMLElement} alert - Alert reference
     * @param {number} animationDuration - Duration of fade out animation in s
     */
    _removeAlert(alert, animationDuration=0.5) {
        /**
         * Create a temporary css class to set the explicit height of the alert
         * to its dynamic height. This is require as css animations cannot
         * transition from height: auto to height: 0 in every browser.
         */
        ...
        // Add attribute to collapse alert
    ...
    /**
     * Show the custom cookie info alert
     */
    showCookieConsentDisclaimer() {
        ...
        agreeButton.addEventListener('click', function (e) {
            ...
            Alerts._removeAlert(alert);
        });

        alertBody.appendChild(agreeButton); // Append
